...
func InsertInitialAccountData(email string, accountId string) {
	...
	if _, err := db.Exec(models.AccountModel.InsertAccount, email, accountId, "NOT_CONNECTED"); err != nil {
		log.Print(err.Error())
	}
}
...
func UpdateAccountData(accountId string, status string) {
	...
	if _, err := db.Exec(models.AccountModel.UpdateAccountState, status, accountId); err != nil {
		log.Print(err.Error())
	}
}

func SelectAccountData(email string) Account {
	res := db.QueryRow(models.AccountModel.GetAccountByEmail, email)
	acc := Account{}
